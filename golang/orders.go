...
// uses the v2.1sandbox endpoint so that the test methods are available
...
func ordersDemo(ctx context.Context, service *content.APIService, config *merchantInfo) {
	...
	// If the endpoint ends in /v2.1/, need to change to /v2.1sandbox/.
	...
	shipReq1 := &content.OrdersShipLineItemsRequest{
		...
		ShipmentInfos: []*content.OrdersCustomBatchRequestEntryShipLineItemsShipmentInfo{
			&content.OrdersCustomBatchRequestEntryShipLineItemsShipmentInfo{
	...
	shipUpdateReq := &content.OrdersUpdateShipmentRequest{
		Status:      "delivered",
		Carrier:     shipReq1.ShipmentInfos[0].Carrier,
		OperationId: newOperationID(),
		ShipmentId:  shipReq1.ShipmentInfos[0].ShipmentId,
		TrackingId:  shipReq1.ShipmentInfos[0].TrackingId,
	}
	...
	shipUpdateReq = &content.OrdersUpdateShipmentRequest{
		Status:      "delivered",
		Carrier:     shipReq2.ShipmentInfos[0].Carrier,
		OperationId: newOperationID(),
		ShipmentId:  shipReq2.ShipmentInfos[0].ShipmentId,
		TrackingId:  shipReq2.ShipmentInfos[0].TrackingId,
	}
	shipUpdateResp, err = orders.Updateshipment(config.MerchantID, orderID, shipUpdateReq).Do()
	if err != nil {
	...
	returnReq := &content.OrdersReturnRefundLineItemRequest{
		LineItemId:  testOrder.LineItems[0].Id,
		OperationId: newOperationID(),
		Quantity:    1,
	...
	returnResp, err := orders.Returnrefundlineitem(config.MerchantID, orderID, returnReq).Do()
	if err != nil {
...
func printOrder(order *content.Order) {
	...
	if customer := order.Customer; customer != nil {
		...
		emailAddress := ""
		if customer.MarketingRightsInfo != nil {
			emailAddress = customer.MarketingRightsInfo.MarketingEmailAddress
		}
		fmt.Printf("  - Email: %s\n", emailAddress)
	}
	fmt.Printf("- Placed on date: %s\n", order.PlacedDate)
	if amount := order.NetPriceAmount; amount != nil {
