...
> [!IMPORTANT]
> The Content API for Shopping is approaching sunset on August 18,
> 2026., and its code samples
> are therefore no longer maintained. We recommend using Merchant API
> instead.
...
samples for that particular language. ere, we cover setting up
...
3. Each set of samples uses a different configuration file. or the Content
    API for Shopping, take the example `merchant-info.json` from the repository
    root and copy it into `$(HOME)/shopping-samples/content`. ext, change its
    ...
    `$(HOME)/shopping-samples/manufacturers`. ext, change its contents
