...
def insert_datafeed_batch(content_api, merchant_id)
  requests = (1..BATCH_SIZE).map do |n|
    example_id = "feed#{unique_id}"
    datafeed = create_example_datafeed(example_id)
    Google::Apis::ContentV2_1::DatafeedsCustomBatchRequestEntry.new(
        batch_id: n,
  ...
  batch_request =
      Google::Apis::ContentV2_1::DatafeedsCustomBatchRequest.new({:entries: requests})

  content_api.custombatch_datafeed(batch_request) do |res, err|
    if err
