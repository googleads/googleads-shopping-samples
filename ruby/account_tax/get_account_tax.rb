...
#
# Gets the tax settings for the specified account.

require_relative 'account_tax_common'

def get_account_tax(content_api, merchant_id, account_id)
  content_api.get_account_tax(merchant_id, account_id) do |res, err|
    if err
      handle_errors(err)
      exit
    end
...
if __FILE__ == $0
  ...
  get_account_tax(content_api, config.merchant_id, account_id)
end
